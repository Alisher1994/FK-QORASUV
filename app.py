...
def dashboard():
    ...
    students_low_balance = Student.query.filter(Student.balance <= 2, Student.status == 'active').count()
...
def students():
    ...
    return render_template('students.html', students=all_students, payment_info=payment_info)
...
def add_payment():
    try:
        ...
        # Пополнить баланс только при полной оплате
        if is_full_payment and lessons_added > 0:
            student.balance += lessons_added
...
def attendance_checkin():
    ...
    try:
        ...
        low_balance = (not student.club_funded and student.balance <= 0)
        ...
        # Списать занятие (только если не клубное финансирование) — допускаем отрицательный баланс
        if not student.club_funded:
            student.balance -= 1
...
def today_attendance():
    ...
    for record in records:
        ...
        low_balance = (not record.student.club_funded) and (record.student.balance <= 0)
        result.append({
            'id': record.id,
            ...
            'balance': record.student.balance,
            'low_balance': low_balance
        })
...
def delete_attendance(attendance_id):
    ...
    # Вернуть занятие ученику
    student = record.student
    if record.lesson_deducted:
        student.balance += 1
    ...
    return jsonify({
        'success': True,
        'message': f'Запись удалена, баланс {student.full_name}: {student.balance}'
    })
...
def get_income_stats():
    ...
    payments = db.session.query(
        Payment,
        Student.full_name.label('student_name')
    ).join(Student).order_by(Payment.payment_date.desc()).limit(50).all()
...
def get_finances_monthly():
    ...
    months = []
    # Последовательность месяцев: январь..декабрь текущего года
